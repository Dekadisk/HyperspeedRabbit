...
ShadowMapVS_SORTIE ShadowMapVS(float4 Pos : POSITION)
{
	...
	Out.Profondeur.x = (Out.Pos.z + 1.0f) / Out.Pos.w;
...
float4 MiniPhongPS(VS_Sortie vs) : SV_Target
{
	...
	float2 PixelActuel = DIMTEX * ShadowCoord;
	...
	if (bTex > 0) {
		...
		couleur = (couleurTexture * VA + couleurTexture * VD + VS * vSMat.rgb) * ValeurOmbre;
	}
	else
		couleur = (vAMat.rgb * VA + vDMat.rgb * VD + VS * vSMat.rgb ) * ValeurOmbre;

	return float4(saturate(couleur), 1.0f);
}

float4 MiniPhongPS1(VS_Sortie vs) : SV_Target0{

	float3 couleur;

	// Normaliser les paramètres 
	float3 N = normalize(vs.Norm);
	float3 L = normalize(vLumiere[1]);
	float3 V = normalize(vs.vDirCam);
	...
	if (bTex > 0)
	{
		...
		couleurTexture = textureEntree.Sample(SampleState, vs.coordTex).rgb;
		// I = A + D * N.L + (R.V)n 
		couleur = couleurTexture * vAEcl[1].rgb +
			couleurTexture * vDEcl[1].rgb * diff +
			vSEcl[1].rgb * vSMat.rgb * S;
	}
	else
	{
		couleur = vAEcl[1].rgb * vAMat.rgb + vDEcl[1].rgb * vDMat.rgb * diff +
			vSEcl[1].rgb * vSMat.rgb * S;
	}
