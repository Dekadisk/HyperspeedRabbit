...
ShadowMapVS_SORTIE ShadowMapVS(float4 Pos : POSITION)
{
	...
	Out.Profondeur.x = (Out.Pos.z - 2.0f) / Out.Pos.w;
...
float4 MiniPhongPS(VS_Sortie vs) : SV_Target
{
	...
	// I = A + D * N.L + (R.V)n
	if(bTex > 0)
		couleur = (couleurTexture * vAEcl.rgb + couleurTexture * vDEcl.rgb * diff.rgb + vSEcl.rgb * vSMat.rgb * S) * ValeurOmbre;
	else
		couleur = (vAMat.rgb * vAEcl.rgb + vDMat.rgb * vDEcl.rgb * diff.rgb + vSEcl.rgb * vSMat.rgb * S) * ValeurOmbre;
