...
namespace PM3D {
	bool CInfoDispositif::isValid() const { return valide; }
	int CInfoDispositif::getWidth() const { return largeur; }
	int CInfoDispositif::getHeight() const { return hauteur; }
	int CInfoDispositif::getMemory() const { return memoire; }

	const wchar_t* CInfoDispositif::getMapName() const { return m_mapName; }
	...
	CInfoDispositif::CInfoDispositif( int NoAdaptateur ) { 
		...
		wcscpy_s(m_mapName, 100, Desc.Description); 
