...
namespace PM3D
{
	...
	HCURSOR CMoteurWindows::m_hCursorArrow = LoadCursorFromFile(L"assets\\cursors\\cursor_rocket.cur");
	HCURSOR CMoteurWindows::m_hCursorTargetUnavailable = LoadCursorFromFile(L"assets\\cursors\\cursor_target_unavailable.cur");
	HCURSOR CMoteurWindows::m_hCursorTargetAvailable =LoadCursorFromFile(L"assets\\cursors\\cursor_target_available.cur");

	HCURSOR* CMoteurWindows::m_hCurrentCursor = &m_hCursorArrow;
...
void CMoteurWindows::quit()
{
	WndProc(hMainWnd, WM_COMMAND, IDM_EXIT, 0);
}

// Set the cursor
void CMoteurWindows::setCursorArrow() noexcept
{
	m_hCurrentCursor = &m_hCursorArrow;
	SetCursor(*m_hCurrentCursor);
}
void CMoteurWindows::setCursorTargetUnavailable() noexcept
{
	m_hCurrentCursor = &m_hCursorTargetUnavailable;
	SetCursor(*m_hCurrentCursor);
}

void CMoteurWindows::setCursorTargetAvailable() noexcept
{
	m_hCurrentCursor = &m_hCursorTargetAvailable;
	SetCursor(*m_hCurrentCursor);
}
...
bool CMoteurWindows::initAppInstance()
{
	...
	hMainWnd = CreateWindow(szWindowClass, szTitle, WS_POPUP,
		0, 0, CW_USEDEFAULT, CW_USEDEFAULT, nullptr, nullptr, hAppInstance, nullptr);
...
void CMoteurWindows::show()
{
	ShowWindow(hMainWnd, SW_SHOWNORMAL);
	UpdateWindow(hMainWnd);
}

bool CMoteurWindows::initSpecific()
{
	// init application
	initAppInstance();
	show();

	return InputManager::get().init(hAppInstance, hMainWnd);
}
...
{
	ID3D11DeviceContext* pImmediateContext = m_pDispositif->GetImmediateContext();
	ID3D11RenderTargetView* pRenderTargetView = m_pDispositif->GetRenderTargetView();

	// On efface la surface de rendu
	float Couleur[4] = { 0.f, 0.f, 0.f, 1.0f };  //  RGBA
	pImmediateContext->ClearRenderTargetView(pRenderTargetView, Couleur);
	// On réinitialise le tampon de profondeur
	ID3D11DepthStencilView* pDepthStencilView = m_pDispositif->GetDepthStencilView();
